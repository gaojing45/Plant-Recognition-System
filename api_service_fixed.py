...
try:
    ...
    print(f"FastAPI相关模块导入失败: {e}")
    print("请安装: pip install fastapi uvicorn slowapi pydantic")
...
if not FASTAPI_AVAILABLE or not PLANTID_AVAILABLE:
    print("依赖模块不完整，无法启动API服务")
    exit(1)
...
async def test_endpoint():
    ...
    return {
        "message": "API服务正常运行",
        "timestamp": datetime.now().isoformat(),
        "plant_identifier_loaded": plant_identifier is not None
    }
